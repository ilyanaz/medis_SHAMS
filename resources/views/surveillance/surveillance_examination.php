...
$patientSectionSeedValues = [
    trim((string) $patientInfoName),
    trim((string) $patientFormValue('diagnosed_history')),
    trim((string) $patientFormValue('medication_history')),
    trim((string) $patientFormValue('admitted_history')),
    trim((string) $patientFormValue('family_history')),
    trim((string) $patientFormValue('others_history')),
    trim((string) $patientFormValue('current_job_title')),
    trim((string) $patientFormValue('smoking_history')),
    trim((string) $patientFormValue('handling_of_chemical')),
];
$medicalHistoryRows = [
...
$sectionStatuses['patient'] = ! empty(array_filter($patientSectionSeedValues, static fn ($value) => $value !== ''));
...
$requestedSectionKey = trim((string) request()->query('section', 'patient'));
...
function computeSectionCompletion(index){switch(index){case 0:return <?php echo $patientInfoName !== '' ? 'true' : 'false'; ?>;case 1:return fieldValue('input[name="company_name"]')!==''&&fieldValue('input[name="chemicals"]')!==''&&fieldValue('select[name="examination_type"]')!==''&&fieldValue('input[name="examination_date"]')!=='';case 2:return everyFilled(['breathing_difficulty','cough','sore_throat','sneezing','chest_pain','palpitation','limb_oedema','drowsiness','dizziness','headache','confusion','lethargy','nausea','vomiting','eye_irritations','blurred_vision','blisters','burns','itching','rash','redness','abdominal_pain','history_abdominal_mass','history_jaundice','diarrhoea','loss_of_weight','loss_of_appetite','dysuria','haematuria'],radioValue);case 3:var clinicalResult=radioValue('result_clinical_findings');return clinicalResult!==''&&(clinicalResult!=='Yes'||fieldValue('textarea[name="elaboration"]')!=='');case 4:return fieldValue('input[name="weight"]')!==''&&fieldValue('input[name="height"]')!==''&&fieldValue('input[name="BMI"]')!==''&&fieldValue('input[name="bp_systolic"]')!==''&&fieldValue('input[name="bp_distolic"]')!==''&&fieldValue('input[name="pulse_rate"]')!==''&&fieldValue('input[name="respiratory_rate"]')!==''&&fieldValue('textarea[name="general_appearances"]')!==''&&fieldValue('input[name="visual_acuity_right"]')!==''&&fieldValue('input[name="visual_acuity_left"]')!==''&&fieldValue('textarea[name="others"]')!==''&&everyFilled(['s1_s2','murmur','ear_nose_throat','colour_blindness','gas_tenderness','abdominal_mass','lymph_nodes','splenomegaly','kidney_tenderness','ballotable','jaundice','hepatomegaly','muscle_tone','muscle_tenderness','power','sensation','sound','air_entry','reproductive','skin'],radioValue);case 5:return everyFilled(['blood_count','renal_function','liver_function','chest_xray'],radioValue)&&fieldValue('input[name="spirometry_FEV1"]')!==''&&fieldValue('input[name="spirometry_FVC"]')!==''&&fieldValue('input[name="spirometry_FEV_FVC"]')!=='';case 6:buildBioPayload();var baselineLines=(baselineStore&&baselineStore.value?baselineStore.value.split(/\r\n|\r|\n/):[]).filter(function(line){return line.trim()!=='';});var annualLines=(annualStore&&annualStore.value?annualStore.value.split(/\r\n|\r|\n/):[]).filter(function(line){return line.trim()!=='';});var existingFiles=activeExistingBloodResultFiles()>0;var newFiles=activeNewBloodResultFiles()>0;if((existingFiles||newFiles)&&(!baselineLines.length||baselineLines.length===annualLines.length)){return true;}if(!baselineLines.length||baselineLines.length!==annualLines.length){return false;}return baselineLines.every(function(line,idx){var parts=line.split('::');return (parts[0]||'').trim()!==''&&(parts[1]||'').trim()!==''&&(annualLines[idx]||'').trim()!=='';});case 7:var fitnessResult=radioValue('fitness_result');return fitnessResult!==''&&(fitnessResult!=='Not fit'||fieldValue('textarea[name="fitness_justification"]')!=='');case 8:return everyFilled(['history_of_health','clinical_findings','CF_work_related','target_organ','TO_work_related','biological_monitoring','BM_work_related','pregnancy_breastFeding','conclusion_fitness'],radioValue);case 9:var selectedRecommendations=recommendationSelected();var hasOtherRecommendation=recommendationOtherToggle&&recommendationOtherToggle.checked&&fieldValue('input[name="recommendation_type_other"]')!=='';return (selectedRecommendations.length>0||hasOtherRecommendation)&&fieldValue('input[name="MRPdate_start"]')!==''&&fieldValue('input[name="MRPdate_end"]')!==''&&fieldValue('input[name="nextReview_date"]')!==''&&fieldValue('input[name="recommendation_employee_signature"]')!==''&&fieldValue('input[name="recommendation_ack_date"]')!=='';default:return false;}}
