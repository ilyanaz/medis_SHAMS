...
$requestedSectionKey = trim((string) request()->query('section', 'chemical'));
...
.file-upload-panel{display:grid;gap:12px;margin-top:16px;padding:16px;border:1px solid #dfe5ea;border-radius:18px;background:#fff}
.file-upload-panel h4{margin:0;font-size:1rem}
.file-upload-panel p{margin:0;color:#64748b;font-size:.85rem}
.file-upload-panel input[type="file"]{width:100%;border:1px dashed #cbd5e1;border-radius:12px;padding:12px;background:#f8fafc;font:inherit}
.file-chip-list{display:flex;flex-wrap:wrap;gap:10px}
.file-chip{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #dbe3ea;border-radius:999px;background:#f8fafc;color:#334155;font-size:.84rem;text-decoration:none}
.file-chip:hover{border-color:#389B5B;color:#166534}
...
function computeSectionCompletion(index){switch(index){case 0:return fieldValue('input[name="company_name"]')!==''&&fieldValue('input[name="chemicals"]')!==''&&fieldValue('select[name="examination_type"]')!==''&&fieldValue('input[name="examination_date"]')!=='';case 1:return everyFilled(['breathing_difficulty','cough','sore_throat','sneezing','chest_pain','palpitation','limb_oedema','drowsiness','dizziness','headache','confusion','lethargy','nausea','vomiting','eye_irritations','blurred_vision','blisters','burns','itching','rash','redness','abdominal_pain','history_abdominal_mass','history_jaundice','diarrhoea','loss_of_weight','loss_of_appetite','dysuria','haematuria'],radioValue);case 2:var clinicalResult=radioValue('result_clinical_findings');return clinicalResult!==''&&(clinicalResult!=='Yes'||fieldValue('textarea[name="elaboration"]')!=='');case 3:return fieldValue('input[name="weight"]')!==''&&fieldValue('input[name="height"]')!==''&&fieldValue('input[name="BMI"]')!==''&&fieldValue('input[name="bp_systolic"]')!==''&&fieldValue('input[name="bp_distolic"]')!==''&&fieldValue('input[name="pulse_rate"]')!==''&&fieldValue('input[name="respiratory_rate"]')!==''&&fieldValue('textarea[name="general_appearances"]')!==''&&fieldValue('input[name="visual_acuity_right"]')!==''&&fieldValue('input[name="visual_acuity_left"]')!==''&&fieldValue('textarea[name="others"]')!==''&&everyFilled(['s1_s2','murmur','ear_nose_throat','colour_blindness','gas_tenderness','abdominal_mass','lymph_nodes','splenomegaly','kidney_tenderness','ballotable','jaundice','hepatomegaly','muscle_tone','muscle_tenderness','power','sensation','sound','air_entry','reproductive','skin'],radioValue);case 4:return everyFilled(['blood_count','renal_function','liver_function','chest_xray'],radioValue)&&fieldValue('input[name="spirometry_FEV1"]')!==''&&fieldValue('input[name="spirometry_FVC"]')!==''&&fieldValue('input[name="spirometry_FEV_FVC"]')!=='';case 5:buildBioPayload();var baselineLines=(baselineStore&&baselineStore.value?baselineStore.value.split(/\r\n|\r|\n/):[]).filter(function(line){return line.trim()!=='';});var annualLines=(annualStore&&annualStore.value?annualStore.value.split(/\r\n|\r|\n/):[]).filter(function(line){return line.trim()!=='';});var fileInput=form.querySelector('input[name="blood_result_files[]"]');var existingFiles=document.querySelectorAll('.file-chip').length>0;var newFiles=!!(fileInput&&fileInput.files&&fileInput.files.length>0);if((existingFiles||newFiles)&&(!baselineLines.length||baselineLines.length===annualLines.length)){return true;}if(!baselineLines.length||baselineLines.length!==annualLines.length){return false;}return baselineLines.every(function(line,idx){var parts=line.split('::');return (parts[0]||'').trim()!==''&&(parts[1]||'').trim()!==''&&(annualLines[idx]||'').trim()!=='';});case 6:var fitnessResult=radioValue('fitness_result');return fitnessResult!==''&&(fitnessResult!=='Not fit'||fieldValue('textarea[name="fitness_justification"]')!=='');case 7:return everyFilled(['history_of_health','clinical_findings','CF_work_related','target_organ','TO_work_related','biological_monitoring','BM_work_related','pregnancy_breastFeding','conclusion_fitness'],radioValue);case 8:var selectedRecommendations=recommendationSelected();var hasOtherRecommendation=recommendationOtherToggle&&recommendationOtherToggle.checked&&fieldValue('input[name="recommendation_type_other"]')!=='';return (selectedRecommendations.length>0||hasOtherRecommendation)&&fieldValue('input[name="MRPdate_start"]')!==''&&fieldValue('input[name="MRPdate_end"]')!==''&&fieldValue('input[name="nextReview_date"]')!==''&&fieldValue('input[name="recommendation_employee_signature"]')!==''&&fieldValue('input[name="recommendation_ack_date"]')!=='';default:return false;}}
