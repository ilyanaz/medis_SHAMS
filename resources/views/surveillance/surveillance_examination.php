...
var privateFileUrlBase='<?php echo $esc(route('private.file.show')); ?>?token=';
...
function hasMedicalRemovalProtection(){var selectedRecommendations=recommendationSelected();var selected=selectedRecommendations.indexOf('Permanent Medical Removal Protection')!==-1||selectedRecommendations.indexOf('Temporary Medical Removal Protection')!==-1;var hasStoredMrpDates=(recommendationMrpStartInput&&(recommendationMrpStartInput.value||'').trim()!=='')||(recommendationMrpEndInput&&(recommendationMrpEndInput.value||'').trim()!=='');return selected||hasStoredMrpDates;}
...
function syncRecommendationDates(){var needsMrpDates=hasMedicalRemovalProtection();if(recommendationMrpStartWrap){recommendationMrpStartWrap.style.display=needsMrpDates?'':'none';}if(recommendationMrpEndWrap){recommendationMrpEndWrap.style.display=needsMrpDates?'':'none';}if(recommendationNextReviewWrap){recommendationNextReviewWrap.style.display='';}if(recommendationMrpStartInput){recommendationMrpStartInput.required=needsMrpDates;}if(recommendationMrpEndInput){recommendationMrpEndInput.required=needsMrpDates;}if(recommendationNextReviewInput){recommendationNextReviewInput.required=false;}}
...
if(bloodResultFileList){bloodResultFileList.querySelectorAll('a[href*="/storage/"]').forEach(function(link){var marker='/storage/';var href=link.getAttribute('href')||'';var position=href.indexOf(marker);if(position===-1){return;}var path=decodeURIComponent(href.slice(position+marker.length));var token=btoa(unescape(encodeURIComponent(path))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');link.href=privateFileUrlBase+token;});}
