...
class PanelController extends Controller
{
    ...
    public function generalReportFolder(Request $request): View|RedirectResponse
    {
        ...
        $viewData['surveillanceReportRows'] = $module === 'surveillance'
            ? $this->folderSurveillanceReportRows($request, $company, $date)
            : [];
        $viewData['audiometryReportRows'] = $module === 'audiometry'
            ? $this->folderAudiometryReportRows($request, $company, $date)
            : [];
    ...
    protected function folderSurveillanceReportRows(Request $request, string $company, string $date): array
    {
        ...
        $examDateExpression = "DATE(COALESCE(declaration.employee_date, declaration.doctor_date))";
        ...
        $query = DB::table('declaration')
            ...
            ->leftJoin('removal_report', 'removal_report.surveillance_id', '=', 'declaration.surveillance_id')
            ->select([
                'declaration.declaration_id',
                'declaration.company_id',
                'declaration.employee_id',
                ...
                'ms_findings.conclusion_fitness',
                'ms_findings.history_of_health',
                'ms_findings.clinical_findings',
                'ms_findings.target_organ',
                'ms_findings.biological_monitoring',
                'ms_findings.CF_work_related',
                'ms_findings.TO_work_related',
                'ms_findings.BM_work_related',
        ...
        if ($clinicId > 0 && ($hasCompanyClinicId || $hasEmployeeClinicId)) {
            $query->where(function ($scoped) use ($clinicId, $hasCompanyClinicId, $hasEmployeeClinicId): void {
                if ($hasCompanyClinicId) {
                    $scoped->orWhere('company.clinic_id', $clinicId);
                }
                if ($hasEmployeeClinicId) {
        ...
        $reportRows = $query->get()->flatMap(function ($row): array {
            ...
            $isAbnormal = in_array((string) ($row->conclusion_fitness ?? ''), ['Not Fit', 'Abnormal'], true)
                || in_array((string) ($row->history_of_health ?? ''), ['Yes', 'Abnormal'], true)
                || in_array((string) ($row->clinical_findings ?? ''), ['Yes', 'Abnormal'], true)
                || in_array((string) ($row->target_organ ?? ''), ['Yes', 'Abnormal'], true)
                || in_array((string) ($row->biological_monitoring ?? ''), ['Yes', 'Abnormal'], true)
                || in_array((string) ($row->CF_work_related ?? ''), ['Yes', 'Abnormal'], true)
                || in_array((string) ($row->TO_work_related ?? ''), ['Yes', 'Abnormal'], true)
            ...
            $hasSavedRemoval = ! empty($row->removal_report_id);
    ...
    protected function folderAudiometryReportRows(Request $request, string $company, string $date): array
    {
        ...
        $clinicId = (int) $request->session()->get('active_clinic_id', 0);
        $audioCommentStatusSelect = DB::raw("CASE WHEN audio_comments.audio_recommendation IS NULL OR audio_comments.audio_recommendation = '' THEN 'pending' ELSE 'completed' END as status");
        $query = DB::table('audiometry_test')
            ->leftJoin('employee', 'employee.employee_id', '=', 'audiometry_test.employee_id')
            ->leftJoin('company', 'company.company_id', '=', 'audiometry_test.company_id')
